...
const Content = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState<string | null>("All projects");
  ...
  const filteredTemplates = templates.filter((template) => {
    const matchCategory =
      selectedCategory === "All projects" || template.category === selectedCategory;
    /* const matchModel =
  ...
  return (
    <div className={cn("w-full space-y-4", "lg:space-y-5")}>
      ...
      <div className={cn("space-y-3", "md:flex md:items-center md:space-y-0 md:gap-2 md:max-w-2xl")}>
        <SearchBar />
          {/* Works */}
              ...
              <div className="absolute left-0 mt-2 w-44 origin-top-right rounded-md bg-background shadow-lg ring-1 ring-white/60 ring-opacity-5 z-50">
                {categories.map((category) => (
                  <div
                    ...
                    className={cn(
                        "py-3 px-4 flex items-center justify-between cursor-pointer transition-colors",
                        selectedCategory === category && "bg-gradient-to-r from-[#5738F4] to-[#7D2AE8] rounded-md"
                    )}
                  >
