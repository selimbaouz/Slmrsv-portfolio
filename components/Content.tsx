...
import { useSnippetsFiltered } from "@/stores/snippetsFiltered";
import SearchBar from "./SearchBar";

const Content = () => {
  const [openCategories, setOpenCategories] = useState(false);
  const [openImagesLinkIndex, setOpenImagesLinkIndex] = useState<number | null>(
    null
  );
  ...
  const categories = ["All projects", "Shopify", "App Canva", "3D", "Website"];
  ...
  return (
    <div className={cn("w-full space-y-4", "lg:space-y-5")}>
      <h2 className={cn("font-semibold text-xl", "lg:text-3xl")}>All works</h2>
      <div
        className={cn(
          "space-y-3",
          "md:flex md:items-center md:space-y-0 md:gap-2 md:max-w-2xl"
        )}
      >
        <SearchBar />
        {/* Works */}
                ...
                {models.map((model) => (
                  <div
                    ...
                    className={cn(
                        ...
                        selectedModel === model && "bg-gradient-to-r from-[#5738F4] to-[#7D2AE8] rounded-md"
                    )}
                  >
        ...
        >
          <button
            ...
            className={cn(
              "cursor-pointer inline-flex gap-3 items-center justify-center w-full rounded-md border shadow-sm px-4 py-2 bg-transparent text-sm font-medium text-white",
              "lg:text-base"
            )}
          >
            Categories
                ...
                <div
                  ...
                  className={cn(
                    ...
                    selectedCategory === category &&
                      "bg-gradient-to-r from-[#5738F4] to-[#7D2AE8] rounded-md"
                  )}
                >
      ...
      <div className="columns-1 gap-4 lg:columns-4 md:columns-2">
        {filteredTemplates.map((data, i) => (
          <div key={i} className="break-inside-avoid mb-4 relative">
            {data.url && (
              <button
                onClick={(e) => {
                  e.preventDefault();
                ...
                className={cn(
                  "absolute top-3 right-2 bg-background/30 p-1 rounded-lg cursor-pointer",
              ...
              >
                <div
                  className={cn(
                    ...
                    "hover:bg-gradient-to-r from-[#5738F4] to-[#7D2AE8] rounded-lg"
                  )}
                >
                  ...
                  <span className="block text-sm">See the project</span>
                </div>
              </div>
            )}
            <Image
